...
#include<cstring>
using namespace std;
#define MYPORT "41074"    // the port users will be connecting to
...
struct StudentRecord {   // a struct to store all the neccessary info corresponding to a student
    string department;
    string studentID;
    vector<int> courseScores;
};
double calculateGPA(const vector<int>& courseScores) {  // function to calcultate the GPA of a student
    double sum = 0.0;
    int count = 0;

    for (int score : courseScores) { // iterate through the scores
...
// function to calculate the mean gpa of a dept 
...
// function to calculate the variance in GPA for a desired department
...
//calculate the min and max GPA in a given dept
void calculateDepartmentMinMaxGPA(const vector<StudentRecord>& studentRecords, const string& department,
    ...
    if (!departmentGPAs.empty()) {
        ...
        std::sort(departmentGPAs.begin(), departmentGPAs.end());
...
string generateDepartmentList(const vector<StudentRecord>& studentRecords) {
   stringstream departmentList;
    ...
    // Create a comma-separated string of department names, for the purpose of sending it to the Main Server
...
int main(void)
{
    ...
    while (getline(inputFile, line)) {
        ...
        for (size_t i = 2; i < tokens.size(); ++i) {
            if (tokens[i] == "none") {
                studentRecord.courseScores.push_back(-1); 
            } else {
                try {
    ...
    /*for (const auto& record : studentRecords) {
        ...
        for (int score : record.courseScores) {
            std::cout << (score == -1 ? "none" : std::to_string(score)) << " ";
        }
        std::cout << "\n\n";
    }*/
   ...
   int sockfd;
    ...
    hints.ai_flags = AI_PASSIVE; // use the   local machine
	...
	// Server A has started!
	cout<<"Server A is up and running using UDP over port 41074"<<endl;
	addr_len = sizeof their_addr;
   recvfrom(sockfd, buf,sizeof(buf) , 0,(struct sockaddr *)&their_addr, &addr_len);
   if(strcmp(buf,"deptlist")==0){
     string  departmentList= generateDepartmentList(studentRecords);  // send the dept list to the main server
	 cout<<"Server A has sent the department list to the Main Server using UDP over port 41074"<<endl;
 ...
 bzero(buffer, sizeof(buffer));
	...
	// handling the desired query that the main server asks and sending it to the main server upon its request
	 if(stuid!=""){
		 cout<<"Server A has received a student academic record query for Student ";
		 cout<<stuid;
		 cout<<" in Department ";
		 cout<<deptname<<endl;
		 auto it = std::find_if(studentRecords.begin(), studentRecords.end(),
		...
		if(it != studentRecords.end()){
		   ...
		   sendto(sockfd, response.c_str(), response.length(), 0, (struct sockaddr *)&their_addr, sizeof(their_addr));
			...
			cout<<"Server A has sent Department: not found to the Main Server using UDP over port 41074"<<endl;
		 sendto(sockfd, response.c_str(), response.length(), 0, (struct sockaddr *)&their_addr, sizeof(their_addr));
		}
		 
	 }
	 else{
		 cout<<"Server A has received a department academic statistics query for Department ";
		 cout<<deptname<<endl;
...
close(sockfd);  // close the socket when not in use
}