...
#include<cstring>
using namespace std;
#define MYPORT "42074"    // the port users will be connecting to
...
int main(void)
{
   ...
   int sockfd;
	...
	cout<<"Server B is up and running using UDP over port 42074"<<endl;
   ...
   if(strcmp(buf,"deptlist")==0){
     string  departmentList= generateDepartmentList(studentRecords);
			 cout<<"Server B has sent the department list to the Main Server using UDP over port 42074"<<endl;
 ...
 bzero(buffer, sizeof(buffer));
	 ...
	 if(stuid!=""){
		 cout<<"Server B has received a student academic record query for Student ";
		 cout<<stuid;
		 cout<<" in Department ";
		 cout<<deptname<<endl;
		...
		if(it != studentRecords.end()){
			...
			cout<<response<<endl;
			cout<<"Server B has sent the result(s) to Main Server"<<endl;
		 ...
		 cout<<"Server B has received a department academic statistics query for Department";
		 cout<<deptname<<endl;
